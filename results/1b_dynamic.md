...
- **project-on-demand** (per-element) here tracks **clamp** (avg 4.8 iters/step), NOT `none` -- a subtle, real distinction: the inertia term (+M/dt²) regularizes the *global assembled* Hessian, but individual *element* Hessians remain indefinite, so a per-element on-demand check still projects them. The faithful **global-pdn** (checks the assembled matrix's definiteness, as in Pitfalls-of-Projection) does exactly what it should: it matches `none` here (avg 3.0 iters/step, full-Newton speed) because inertia makes the assembled Hessian SPD, yet stays safe in statics (E1: it converges via a fallback shift where `none` fails). This global-vs-per-element split is a real filter-design axis the harness now measures directly.
