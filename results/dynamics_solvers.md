...
- **`anderson-geometry → chebyshev-semi-iterative` (convergence) — REPRODUCES (modest):** accelerating the SAME PD fixed point, Anderson (m=5) reaches the tol in **6** iterations vs Chebyshev's **7** — Anderson is marginally faster and needs no spectral-radius estimate (Chebyshev's ρ is hand-tuned).
- **`chebyshev-semi-iterative → nonlinear-conjugate-gradient` (convergence) — REPRODUCES:** with the SAME A0 preconditioner, Chebyshev reaches the tol in **7** iterations and preconditioned nonlinear-CG in **8** — CG does NOT exceed Chebyshev (it is one iteration slower), and CG additionally pays ~2 inner products per iterate for its adaptive β where Chebyshev uses fixed weights. Both halves of the claim hold. (Un-preconditioned CG needs ~140 — the comparison is only fair with the shared proxy.)
