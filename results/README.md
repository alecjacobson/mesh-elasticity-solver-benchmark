...
| INJ | injectivity/feasibility — untangle a folded init (barrier vs barrier-free) | [`injectivity.md`](injectivity.md) | **capability axis:** barrier symmetric-Dirichlet **0% feasible-start** from folds (+∞ at J≤0 — needs a Tutte-style feasible init), while barrier-free energies untangle **100%** across severities — the area-penalty (TLC's ancestor, 30–52 it) and Stable NH (**8–9 it**, far better basin). Establishes the untangle-from-folds capability separating the World-1 injectivity cohort from distortion-barrier minimizers |
| E3 ⭐ | BCQN triple-split — **full 2³ factorial** (line-search × direction × criterion) | [`e3.md`](e3.md) | one unified solver over all 3 factors: the **Sobolev direction is the only real lever** (~20–32% fewer iters, growing with ill-conditioning; within noise on near-inversion), the **barrier line-search** is an energy-eval tweak redundant with the barrier energy, the **characteristic criterion** just re-times the stop (±few it) — BCQN is **one strong + two minor** factors, not three co-equal; `bcqn→{aqp,slim,l-bfgs}` qualified, attributed per factor/regime |
