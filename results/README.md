...
| 3D-tw | **Adversarial 3D torsion — filter necessity is regime-dependent** | [`tet3d_twist.md`](tet3d_twist.md) | far face twisted to ~69° on the 3D harness. Under smooth torsion from a valid start **raw Newton converges on every level and is FASTEST** (3–6 it) vs clamp 4–9 / absolute 5–13 — so "filtering is necessary" (§8.6) is a statement about the *far-from-min/near-inversion regime*, not the dimension. Honest nuance, no new edge |

| P2-3D ⭐ | **§8.1 completed in 3D — locking-relieved quadratic tet** | [`tet3d_p2.md`](tet3d_p2.md) | 10-node P2 tet (`bench/tet_p2.py`, gate 17: rigid invariance, grad & element-Hessian vs FD). ν-sweep at ν=0.499: **P1 locks** (clamp 267, absolute 300+ — absolute worse, the reversal) but **P2 relieves it** (clamp 19, absolute 17 — **absolute now BEATS clamp**), so the 3D P1 reversal is a volumetric-locking artifact, exactly as in 2D. Flagship confound-control result now complete in 3D |
...
| IPC ⭐ | **World-3 opened — minimal faithful 2D IPC intersection-free guarantee** | [`ipc.md`](ipc.md) | faithful C² log-barrier b(d)=-(d-d̂)²ln(d/d̂) + CCD-filtered line search + implicit-Euler Newton (`bench/ipc.py`, gate 16: barrier shape, b'/b'' vs FD, guarantee). Body thrown into a wedge at speeds 4–30: **IPC keeps min wall-distance >0 at every speed** (0.013–0.016, intersection-free by construction) while a classical penalty **tunnels 5/5** (−0.09 to −0.28). `ipc→prior-rigid-engines` unmeasured→qualified — first World-3 edge |

| TLC | **Faithful Total Lifted Content** (Du et al. 2020) vs its α=0 ablation | [`tlc.md`](tlc.md) | reimplemented from paper + reference code (Cayley-Menger lifted-content energy, Tutte auxiliary, auto α, L-BFGS to first injectivity), conformance-gated (gate 15: barrier-free at folds, grad vs FD, α→0==TUA, untangles). On 8 folded maps: **TLC untangles 6/8 vs its α=0 limit (TUA) 1/8** (`tlc→tua` reproduces Prop 4.3 — the lifting makes minima injective); barrier symmetric-Dirichlet 0/8 (can't start). 1 edge → qualified |
