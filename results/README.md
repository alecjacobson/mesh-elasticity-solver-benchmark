...
| W2filt | filter head-to-head (clamp/absolute/trust-region, P1 vs P2) | [`world2_filters.md`](world2_filters.md) | trust-region uses **fewer iterations** but is **~4× slower in wall-clock** (full assembled `eigh`/step) — the round-1 'validated/dominates' claim was iteration-only and is **withdrawn** (review-r2); paired iters+wall+factorization view reported |
...
| W1prof | World-1 data profiles | [`world1_profiles.md`](world1_profiles.md) | pairwise (not a total order): Newton & Sobolev-L-BFGS reach the energy-tol soonest, AQP needs the largest budget; single-run/no error bars — descriptive (#48/#49) |
| AA | Anderson vs local-global + generality | [`anderson.md`](anderson.md) | Anderson **12 it vs 23 it** to the same non-trivial minimum, mesh-independent; **wall-clock speedup < iteration speedup** (lstsq overhead); **generality:** the same AA core also accelerates a Jacobi linear solve **374→62→37 it** (m=0/5/10) — property of the generic core, not the map (#36) |
...
| MI | AQP mesh-independence (design regime) | [`mesh_independence.md`](mesh_independence.md) | AQP count ~flat (9→6→6) over 5.6× DOFs while L-BFGS grows 16→26→37 — **consistent with** AQP's mesh-independence design claim, but **suggestive only** (3 sizes, 1 seed, non-monotone; review-r2 #52) |
