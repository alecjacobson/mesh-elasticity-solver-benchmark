...
| E3 | BCQN triple-split — **line-search main effect** (barrier-aware vs backtracking) | [`e3.md`](e3.md) | isolating BCQN's cheapest factor: barrier-aware LS cuts energy-evals only **modestly** (67→56 adversarial, single-digit %, **not the >10× of Fig.6**) and **success is 5/5 both** — because symmetric Dirichlet is *itself* a barrier (+∞ at J≤0), so the component is **partly redundant** with the energy's own barrier; `bcqn→{l-bfgs,aqp}` stays **qualified** (energy/regime-dependent) |
