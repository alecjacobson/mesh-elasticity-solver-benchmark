...
UNI = {
    "‖": r"\|", "²": r"\textsuperscript{2}", "³": r"\textsuperscript{3}", "⁻¹": r"\textsuperscript{-1}",
...
def esc(s):
    for k, v in UNI.items():
        s = s.replace(k, v)
    # escape LaTeX specials that survive (avoid double-escaping already-inserted commands is hard;
    # this paper's prose is light on raw &,%,#,_ outside code — handle the common ones)
...
def convert_block(md):
    ...
    while i < len(lines):
        ...
        if re.match(r"^#\s", ln):
            ...
            m = re.match(r"!\[([^\]]*)\]\(([^)]+)\)", ln)
            cap, path = m.group(1), m.group(2).replace("../", "")
...
def main():
    ...
    abs_body = inline(m.group(1).strip().split("\n\n")[0]) if m else ""
