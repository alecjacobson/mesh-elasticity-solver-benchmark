...
| ![injectivity](injectivity.png) `injectivity` | **Injectivity is a capability axis.** A folded map (108 inverted, red) untangled to all-valid by two barrier-free energies — the classical area penalty (57 it) and Stable NH (9 it) — while barrier symmetric-Dirichlet is +∞ at folds (0% feasible start). See `results/injectivity.md`. |
...
| ![perf1a](perf_profiles_1a.png) `perf_profiles_1a` | **The full 1a accelerator suite** (Newton/L-BFGS/Sobolev-L-BFGS/AQP, shared symmetric-Dirichlet E*) over 18 problems (easy/typical/ill-conditioned × 2 meshes × 3 seeds): Dolan–Moré performance profile + Moré–Wild data profile. Newton dominant; AQP's first-order tail lengthens at tight τ. See `results/1a_profiles.md`. |
