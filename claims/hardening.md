...
| trust-region-filtering → clamp-filtering | convergence | **qualified (indicative)** | 'validated' WITHDRAWN (review-r2 #42): TR uses ~1.3× fewer ITERATIONS but ~4× MORE WALL-CLOCK than clamp on P1 (full assembled eigh/step vs per-element 6×6). Reduces iterations (≤both on P2) but trades for per-step cost; P1/P2 use different TR impls; single scenario/τ (world2_filters, #38) |
| trust-region-filtering → absolute-filtering | convergence | **qualified (indicative)** | 'validated' withdrawn (review-r2): fewer iterations than absolute but slower wall-clock (assembled eigh); see the paired trust-region→clamp row for cost accounting (world2_filters) |
