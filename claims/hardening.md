...
| trust-region-filtering → clamp-filtering | convergence | **validated** | faithful three-state blend (reproduces Newton/clamp/abs exactly, conformance-gated) beats clamp on BOTH P2 (39 vs 53) and P1 (139 vs 242) — the restored w=0 full-Newton branch wins on locking P1 (world2_filters, #38) |
| trust-region-filtering → absolute-filtering | convergence | **validated** | TR beats absolute on BOTH P2 (39 vs 41) and P1 (139 vs maxiter); no longer degrades to absolute on locking P1 (world2_filters, #38) |
