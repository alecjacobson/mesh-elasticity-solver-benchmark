...
| trust-region-filtering → clamp-filtering | convergence | **qualified (discretization-dependent)** | ρ-switchboard, per-element, principled schedule (SPD-probe; review-r3): on locking **P1 it WINS both axes** (120 it/8.0s vs clamp 242/11.1s); on locking-free **P2 it's a WASH** — iteration-parity with clamp (52 vs 53) but ~1.8× slower wall-clock (escalation overhead), beaten by absolute. Adaptivity helps only when the problem is hard (world2_filters) |
| trust-region-filtering → absolute-filtering | convergence | **qualified (discretization-dependent)** | TR beats absolute on locking P1 (absolute maxiters); on locking-free P2 absolute is best (41 vs TR 52 it, less wall-clock). See paired trust-region→clamp row (world2_filters) |
...
| pitfalls-projection → clamp-filtering | convergence | **qualified (affine-invariance sub-claim validated)** | **affine invariance broken by projection, shown directly** (pitfalls): unfiltered Newton covariant to 3e-13 under coordinate rescaling; clamp/absolute/global-PDN all O(1). Rate: clamp inert→super-linear in a clean SPD basin (honest null). Paper's 'PDN recovers invariance' holds only where PDN is inert (SPD); 1b-dynamic consistent |
