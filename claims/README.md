...
- `trust-region-filtering → {clamp, absolute}` — **qualified (discretization-dependent)**: a ρ-switchboard (per-element, principled SPD-probe schedule) **wins on the locking P1** element (both iterations and wall-clock: 120 it/8.0s vs clamp 242/11.1s) but is **at best a wash on the well-conditioned P2** (iteration-parity with clamp 52 vs 53, wall-clock-penalized, beaten by absolute) — the adaptivity helps only when the problem is hard. Round-1's "beats both on P2" was a costlier-`eigh` artifact, reversed (`results/world2_filters.md`).
