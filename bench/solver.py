...
`filter` and holds the rest fixed.
"""
import time
import numpy as np
from .energy import element_terms
from .filters import project_element, PER_ELEMENT


def assemble(x, tris, Bs, areas, filt):
    ...
    for t, tri in enumerate(tris):
        dofs = np.array([2 * tri[0], 2 * tri[0] + 1,
                         2 * tri[1], 2 * tri[1] + 1,
                         2 * tri[2], 2 * tri[2] + 1])
        Ee, ge, He, _ = element_terms(x[dofs], Bs[t], areas[t])
...
def energy_only(x, tris, Bs, areas):
    E = 0.0
    for t, tri in enumerate(tris):
        dofs = np.array([2 * tri[0], 2 * tri[0] + 1, 2 * tri[1], 2 * tri[1] + 1,
                         2 * tri[2], 2 * tri[2] + 1])
        Ee, _, _, _ = element_terms(x[dofs], Bs[t], areas[t])
        if not np.isfinite(Ee):
            return np.inf
        E += Ee
    return E
...
def solve(x0, tris, Bs, areas, free, filt, max_iter=300, tol=1e-6, c=1e-4):
    """Run projected Newton with the given filter. Returns a result dict + per-iter log."""
    x = x0.copy()
    log = []
    t0 = time.perf_counter()
    status = "maxiter"
    for it in range(max_iter):
        E, g, H = assemble(x, tris, Bs, areas, filt)
        ...
        # backtracking Armijo line search on the global energy (inversion -> inf -> rejected)
        alpha = 1.0
        xf0 = x[free].copy()
        while True:
            x[free] = xf0 + alpha * d
            En = energy_only(x, tris, Bs, areas)
    ...
    return {"filter": filt, "status": status, "iters": len(log) - (1 if status == "converged" else 0),
