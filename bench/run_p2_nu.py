...
def main():
    ...
    lines += ["", "## Observed -- the ν-claim is a discretization artifact on P1", "",
              ...
              "- **Conclusion:** the earlier 'absolute is worse' was **entirely a volumetric-"
              "locking artifact of the P1 element**, not a property of the filter. Using a proper "
              "(locking-free-er) discretization *reverses* the conclusion and vindicates both the "
              "paper's claim AND the benchmark's control C1. This is the benchmark doing its job: "
              "**separating a real solver effect from a discretization confound** -- the exact "
              "failure mode the survey exists to catch.",
              "",
