...
| **P3 — paper + release** | write the STAR; release harness as living-benchmark seed (closed/open divisions, hidden tier) | 🟡 **draft** — full STAR first draft in [`paper/paper.md`](paper/paper.md) (§1–§11, ~7.7k words: unifying view, taxonomy, survey-by-axis, lineage, claims graph, benchmark design, the decomposition results, what-survived + review-loop-as-method, living-benchmark plan). Every quantitative claim cited to a `results/*.md`; reference-integrity checked. Remaining: adversarial review pass on the prose, LaTeX conversion, and the release-seed division/reproducibility spec. |
