...
conformance-gated [`bench/`](bench/) harness covering **all six taxonomy axes** and **30 measured experiments** ([`results/`](results/)) with **20 deterministic figures** ([`figures/`](figures/)). See [Status & roadmap](#status--roadmap).
...
| [`paper/`](paper/) | **The STAR draft** — assembled [`paper.md`](paper/paper.md) (~7.7k words, §1–§11, every number cited to `results/`) from markdown section files; `python paper/assemble.py` |
...
| **P2 — 1a + feasibility** | distortion accelerators + injectivity suites; BCQN triple-split (E3); full performance profiles | 🟢 **structurally complete** — all four deliverables landed and adversarially reviewed: **(E3)** the full 2³ BCQN factorial (`bench/e3_solver.py`, `results/e3.md`) — the components **entangle, not add** (the barrier line-search cancels the Sobolev direction); **(1a accelerator suite + full performance profiles)** Dolan–Moré + Moré–Wild + Gould–Scott pairwise over 18 problems on a shared E\* (`bench/run_1a_profiles.py`, `figures/perf_profiles_1a.png`) — Newton dominant, AQP's tail lengthens at tight τ, Sobolev a pooled-wash but 100% in its ill-conditioned regime; **(feasibility suite)** barrier-vs-barrier-free untangling of folded inits + a non-convex hard-boundary probe (`bench/{untangle,run_injectivity}.py`, `figures/injectivity.png`); plus the **twist-eigenvalue analysis** (`results/twist_analysis.md`). **Deferred (need source papers, like #14 CM):** faithful per-method ports to rank *within* the injectivity cohort (TLC lifted-content, foldover-free) and Composite Majorization (#14) — within-cohort refinements, not structural P2 blockers. |
| **P3 — paper + release** | write the STAR; release harness as living-benchmark seed (closed/open divisions, hidden tier) | 🟡 **draft** — full STAR first draft in [`paper/paper.md`](paper/paper.md) (§1–§11, ~7.7k words: unifying view, taxonomy, survey-by-axis, lineage, claims graph, benchmark design, the decomposition results, what-survived + review-loop-as-method, living-benchmark plan). Every quantitative claim cited to a `results/*.md`; reference-integrity checked, and adversarially reviewed (two rounds — claims-ledger accounting + bibliography/evidence integrity, no fabrication found). Now with a **bibliography** ([`paper/references.bib`](paper/references.bib), derived from claims.yaml + curated classical ancestors), a **compiled PDF** ([`paper/paper.pdf`](paper/paper.pdf), 25 pp — built by [`paper/build.sh`](paper/build.sh) via pdflatex+bibtex, tracked with **Git LFS**), the **living-benchmark release seed** ([`docs/living-benchmark.md`](docs/living-benchmark.md)), and a **multi-config-hardened headline** (absolute beats clamp 5/5 across diverse stretches/shears). Remaining P3 tail: inline `\cite` wiring + a venue class (the PDF compiles cleanly with `\nocite{*}`); DOI backfill (#100). |
